...
def  valueIteration(mdp):
    ...
    columns = []
    for state in mdp.states():
            optimalValue.append('{:.2f} ({})'.format(V[tuple(state)],str(pi[tuple(state)])))
    cell_text_value = np.array(optimalValue).reshape((2*mdp.Xmax+1,2*mdp.Vmax+1))
    ...
    plt.axis('tight')
    plt.axis('off')
    plt.table(cellText = cell_text_value, rowLabels=(rows), colLabels=(columns), loc=('center'))    
    # plt.table(cellText = cell_text_value, rowLabels=(rows), colLabels=(columns), loc=('center')) 
    plt.savefig('OptimalValueAndPolicy.png', dpi=300)
